...
import comprensoes.VideoWriter;
import comprensoes.testDCT;
import java.io.FileInputStream;
import java.nio.ByteBuffer;
import model.Imagem;
import openGL.openGLPlayer;
...
public class CarregarView extends javax.swing.JFrame {
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION)
            {
                if (DEBUG==false){
                    ...
                    while (true){ // Procura para ver quantos arquivos tem
                        String nomeProcurado = dir.getPath()+"//"+basicoNome+retornarStringPorNumero(id)+extensao;
                    ...
                    */
                    ...
                    byte[] tamAux = new byte[4];
                    ByteBuffer bb = ByteBuffer.wrap(tamAux);
                    fi.read(tamAux);
                    int tam = bb.getInt();
                    System.out.println("tamanho imagem : " + tam);
                    
                    byte[] imagem = new byte[tam+2];
                    fi.read(imagem);
                    Imagem img = testDCT.carregarImagem(imagem);
                    //System.out.println("saida img : " + img.testPrint());
                    
                    JFrame frame = new JFrame();
                    imagemPanel p = new imagemPanel(img);
                    frame.add(p);
                    frame.pack();
                    frame.setVisible(true);
                }
    ...
    private void btAbrirVideoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btAbrirVideoActionPerformed
        ...
        try{
            ...
            {
                File file = fcBuffer.getSelectedFile();
                openGLPlayer player = new openGLPlayer();
                player.playVideo(file,openGLPlayer.DCT,50);
                /*
                File file = fcBuffer.getSelectedFile();
